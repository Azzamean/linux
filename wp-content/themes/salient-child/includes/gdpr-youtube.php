...
Version:		1.7.1
Plugin URI:		https://www.altmann.de/blog/code-snippet-gdpr-compliant-youtube-videos/ and https://www.altmann.de/en/blog-en/code-snippet-gdpr-compliant-youtube-videos/
...
--------------------------------------------------------------------------------------------------------------
PHP 		7.4, 8.1, 8.2
WordPress	6.4.2, 6.4.3, 6.5, 6.5.2, 6.5.5
Bricks		1.9.5, 1.9.6, 1.9.7, 1.9.8, 1.9.9
Oxygen		4.8.1, 4.8.2, 4.8.3
--------------------------------------------------------------------------------------------------------------
...
class MA_GDPR_YouTube {
	...
	private function retrieve_video_info(string $id, string $url, string $lang, object $info): ?object {
		...
		if ($html = @file_get_contents($url,false,$context)) {
			...
			//@file_put_contents(str_replace('.json','.html',$info_file_path),$html);
	...
	private function get_video_info(string $id, string $lang): ?object {
		...
		// either fiel doesn't exist yet, or info or language missing. Retrieve from YouTube.
