...
if (
    ...
    if (have_posts()):
	  ...
	  <div class="row heading-title-omp hentry" data-header-style="<?php echo esc_attr(
       $blog_header_type
   ); ?>">
		<div class="col span_12 section-title-omp blog-title">
				<?php if (
        $blog_header_type === "default_minimal" &&
        "post" === get_post_type()
    ) { ?>
     ...
     if (!empty($categories)) {
         ...
         foreach ($categories as $cat) {
             $output .=
                 $comma .
                 '<a class="categories-techstrongtv" href="' .
         ...
         echo apply_filters(
             "nectar_blog_page_header_categories",
             trim($output)
         ); // WPCS: XSS ok.
     }
     ?>
   ...
   if ("post" === get_post_type()) {
       ...
       if (
           function_exists("nectar_social_sharing_output") &&
           "default" === $blog_social_style
       ) {
           $using_fixed_salient_social = "true";
       }
       ?>
			<div id="single-below-header-omp" data-hide-on-mobile="<?php echo esc_attr(
    ...
    $remove_reading_dur = isset(
        $nectar_options["blog_remove_single_reading_dur"]
    )
        ? $nectar_options["blog_remove_single_reading_dur"]
        : "0";
    if ("1" !== $remove_reading_dur) {
        echo '<span class="meta-reading-time">' .
            nectar_estimated_reading_time(get_the_content()) .
            " " .
            esc_html__("min read", "salient") .
            "</span>";
    }
    ?>
       ...
       "post" === get_post_type()
			...
			<div id="single-meta">
				...
				<div class="meta-comment-count">
				  <a href="<?php comments_link(); ?>"><i class="icon-default-style steadysets-icon-chat-3"></i> <?php comments_number(
    esc_html__("No Comments", "salient"),
    esc_html__("One Comment ", "salient"),
    esc_html__("% Comments", "salient")
); ?></a>
				</div>

					<?php if ($blog_social_style != "fixed") {
