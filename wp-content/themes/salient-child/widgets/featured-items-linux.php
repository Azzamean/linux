...
class featuredItems_widget extends WP_Widget {
	...
	function __construct() {	
		parent::__construct(
			...
			__('Linux Foundation Featured Items', 'featuredItems_widget_domain'), 
			array( 'description' => __( 'Widget for featured item.', 'featuredItems_widget_domain' ), )
		);	
	}
	 
	public function widget( $args, $instance ) {
		$category = apply_filters( 'widget_category', $instance['category'] );
		$category_url = apply_filters( 'widget_category_url', $instance['category_url'] );
		...
		$image_url = apply_filters( 'widget_image_url', $instance['image_url'] );
			...
			$output .= '<div class="list-design flex">';
				...
				$output .= '<div class="list-design right">';
					$output .= '<a href="' . $category_url . '"><p class="widget-category">' . $category  . '</p></a>';
					$output .= '<a href="' . $title_url . '"><h3 class="widget-title">' . $title  . '</h3></a>';
					$output .= '<p class="widget-date">' . date_format($date,"F j, Y") . '</p>';
				$output .= '</div>';
	...
	public function form( $instance ) {
		...
		?>	
			...
			<p>
			<label for="<?php echo $this->get_field_id( 'category_url' ); ?>"><?php _e( 'Category URL' ); ?></label>
			<input class="widefat" id="<?php echo $this->get_field_id( 'category_url' ); ?>" name="<?php echo $this->get_field_name( 'category_url' ); ?>" type="text" value="<?php echo esc_attr( $category_url ); ?>" />
			</p>
			...
			<input type="date" class="widefat" id="<?php echo $this->get_field_id( 'date' ); ?>" name="<?php echo $this->get_field_name( 'date' ); ?>" type="text" value="<?php echo esc_attr( $date ); ?>" />
			</p>		
					
			<label class="widg-label widg-img-label" for="<?php echo $this->get_field_id( 'image_url' ); ?>">Image</label>
			<div class="widg-img" >
				<img class="<?php echo $this->get_field_id( 'image_id' ); ?>_media_image custom_media_image" src="<?php if( !empty( $instance['image_url'] ) ){echo $instance['image_url'];} ?>" />
				...
				<input type="text" class="<?php echo $this->get_field_id( 'image_id' ); ?>_media_url custom_media_url" name="<?php echo $this->get_field_name( 'image_url' ); ?>" id="<?php echo $this->get_field_id( 'image_url' ); ?>" value="<?php echo $instance['image_url']; ?>" >
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance['category_url'] = ( ! empty( $new_instance['category_url'] ) ) ? strip_tags( $new_instance['category_url'] ) : '';
