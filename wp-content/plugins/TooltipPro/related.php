...
class CMTT_Related {
    ...
    public static function init() {
        ...
        add_filter('cmtt_add_backlink_content', array(__CLASS__, 'maybeAddRelatedArticles'), 10, 2);
        ...
        if ($excludeFromRelated) {
            ...
            if (!empty($templatesArr)) {
                ...
                foreach ($glossaryIds as $glossaryId) {
                    $record_exists = false;
                    if (\CM\CMTT_Settings::get('cmtt_additional_check_related_articles', 0)) {
                        $record = $wpdb->get_row(
    ...
    public static function triggerOnView($post_id, $content) {
        ...
        if (class_exists('CMTT_Glossary_Plus')) {
            $result = update_post_meta($post_id, self::TRANSIENT_NAME, $replacedTerms);
        }
    }
    ...
    public static function renderRelatedArticles($glossaryId, $limitArticles = 5, $limitGlossary = 5, $force = false, $showPagination = false, $limitCustomArticles = 2) {
        ...
        $tag = 'div';
        if (count($articles) > 0) {
            $html .= '<div class="cmtt_related_articles_wrapper">'; // div.cmtt_related_articles_wrapper
    ...
    public static function renderRelatedTerms($terms) {
        ...
        if (is_array($terms) && count($terms) > 0) {
            ...
            foreach ($terms as $term) {
                ...
                $permalink = apply_filters('cmtt_term_tooltip_permalink', get_permalink($term['post']->ID), $term['post']->ID);
                /*
