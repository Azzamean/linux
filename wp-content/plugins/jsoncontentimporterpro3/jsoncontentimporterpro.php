...
define( 'JCIPRO_VERSION', '3.8.0' ); // current version number
...
define( 'EDD_JCIPRO_ITEM_ID', 296);
define( 'JCI_DEFAULTPARSER', 'twig351adj');
...
if (get_option('jci_pro_load_jquery') == 1) {
	...
	function jci_enqueue_jquery() {
		if (jci_enqueue_onpageids()) {
			wp_enqueue_script( 'jci_enqueue_jquery', plugins_url('/js/jquery/jquery-3.6.1.min.js', __FILE__) );
		}
	}
}
...
if (get_option('jci_pro_load_leaflet') == 1) {
	add_action( 'wp_enqueue_scripts', 'jci_enqueue_leaflet' );
	function jci_enqueue_leaflet() {
		if (jci_enqueue_onpageids()) {
			wp_enqueue_style( 'jci_enqueue_leaflet', plugins_url('/js/leaflet/leaflet/1.9.1/leaflet.css', __FILE__) );
			wp_enqueue_script('jci_enqueue_leaflet', plugins_url('/js/leaflet/leaflet/1.9.1/leaflet.js', __FILE__) );
			wp_enqueue_style( 'jci_enqueue_leaflet_markerCluster', plugins_url('/js/leaflet/MarkerCluster/1.4.1/MarkerCluster.css', __FILE__) );
			wp_enqueue_style( 'jci_enqueue_leaflet_markerCluster_default', plugins_url('/js/leaflet/MarkerCluster/1.4.1/MarkerCluster.Default.css', __FILE__) );
			wp_enqueue_script('jci_enqueue_leaflet_markercluster', plugins_url('/js/leaflet/MarkerCluster/1.4.1/leaflet.markercluster.js', __FILE__) );
		}
	}
}
## END: Load Libs
...
function edd_sl_jcipro_plugin_updater() {
	...
	$edd_updater = new EDD_SL_Plugin_Updater( EDD_JCIPRO_STORE_URL, __FILE__, array(
			...
			'item_id' => EDD_JCIPRO_ITEM_ID, 	// id of this plugin
