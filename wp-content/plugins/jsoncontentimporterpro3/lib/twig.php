...
class doJCITwig {
	...
	private $isTwig351adj = false;
	private $parser = "twig351adj"; #"twig";
	...
	public function tryTwig(){
		if ($this->isTwig2 || $this->isTwig3 || $this->isTwig332) {
			...
			*/
		} else if ($this->isTwig332adj || $this->isTwig351adj) {
			try {
				$this->twig_environment->parse($this->twig_environment->tokenize($this->ts));
	...
	private function jci_setup_twig_environment() {
		...
		# switch deprecated twig-versions to JCI_DEFAULTPARSER
		#echo $this->parser;
		if (
			($this->parser=="twig243")
			|| ($this->parser=="twig")
			...
			$this->parser = JCI_DEFAULTPARSER;
			$this->isTwig351adj = TRUE; 
		}
		#echo $this->parser;
			...
			if ($this->parser=="twig243") {
			...
			} else if ($this->parser=="twig351adj") {
				$inc = WP_PLUGIN_DIR . '/jsoncontentimporterpro3/twiglib/twig351adj/vendor/autoload.php';
				require_once $inc;
				$this->isTwig351adj = TRUE; 
				...
				$this->isTwig351adj = FALSE; 
				$this->isJCIParser = TRUE;
			} else if ($this->parser=="twig") {
	...
	private function twig_ext_dateformat($data, $dateformatstr, $datetimezone, $datelocale) {
			...
			if ((!empty($data)) && (!( is_numeric($data) && (int)$data == $data ))) {
				$data = strtotime($data);
			}
	...
	private function twig_ext_get_attachment_image_src($data, $size = 'thumbnail', bool $icon = false) {
		$gh = wp_get_attachment_image_src($data, $size, $icon);
		return $gh;
	}
	
	private function twig_ext_get_headers($data, bool $associative = false, \resource $context = null) {
		$gh = get_headers($data, $associative , $context);
		return $gh;
	}
	...
	private function jci_utf8_decode($utf8_string) { # php 8.2: utf8_decode removed, deprecated
	...
	private function twig_ext_removespecialcharsinurl($data) {
         ...
         $data = str_replace(array('ä','ö','ü','ß','Ä','Ö','Ü', '&auml;', '&ouml;', '&uuml;', '&szlig;', '&Auml;', '&Ouml;', '&Uuml;'),array('ae','oe','ue','ss','Ae','Oe','Ue', 'ae', 'oe', 'ue', 'ss', 'Ae', 'Oe', 'Ue'),$this->jci_utf8_decode($data)); 
	...
	private function twig_extra_function_woocommmerce_calc_authorization($consumer_key, $consumer_secret, $request_uri, $http_method) {
		...
		# if the URL comes with a query-str the query-string has to be part of the ORDERED $query_string
		$request_uri_tmp = explode("?", $request_uri, 2);
		$base_request_uri = rawurlencode( $request_uri_tmp[0] );
		if (isset($request_uri_tmp[1]) && (""!=$request_uri_tmp[1])) {
			$paramsQS = explode("&", $request_uri_tmp[1]);
			foreach ($paramsQS as $p) {
				$ptmp = explode("=", $p);
				if (isset($ptmp[1]) && (""!=$ptmp[1])) {
					$paramswoosig[$ptmp[0]] = $ptmp[1];
				} else {
					$paramswoosig[$ptmp[0]] = "";
				}
			}
			ksort($paramswoosig);  #woo expects this order!
		}
		$query_string = $this->jci_join_params( $paramswoosig );
	...
	private function twig_ext_create_new_custom_post($posttype, $titel="set title in twig, please", $name="set name in twig, please", $content="set content in twig, please",  $excerpt="", $publishdate="", $postStatusUsed="publish", $authorid="", $debug=FALSE) {
		...
		$publishdate = $this->executeTwig($this->jsonObj, $publishdate, JCI_DEFAULTPARSER, FALSE); # required format is Y-m-d H:i:s 
		...
		$titel = $this->executeTwig($this->jsonObj, $titel, JCI_DEFAULTPARSER, FALSE);
		$this->debug_out("create CP - titel", $titel); 

		$name = $this->executeTwig($this->jsonObj, $name, JCI_DEFAULTPARSER, FALSE);
		$this->debug_out("create CP - name", $name); 

		$content = $this->executeTwig($this->jsonObj, $content, JCI_DEFAULTPARSER, FALSE);
	...
	private function twig_ext_get_taxonomy($pageid, $taxonomySlug="", $debug=FALSE) {
		$this->debug = $debug;
		$this->debug_out("get taxonomy", "pageid: $pageid / taxonomy: $taxonomySlug"); 
		$terms = get_the_terms( $pageid, $taxonomySlug );
		if (!$terms) {   # e.g. invalidf pageid return false
			$returnArr = Array();
			$returnArr["success"] = $terms;
	...
	private function twig_ext_get_cp_by_cpf_keyvalue($post_type="", $key="", $value="", $debug=FALSE) {
		...
		$this->debug = $debug;
		$value = $this->executeTwig($this->jsonObj, $value, JCI_DEFAULTPARSER, FALSE);
	...
	private function twig_ext_get_custom_field_value($pageid, $key="", $debug=FALSE) {
		...
		$key = $this->executeTwig($this->jsonObj, $key, JCI_DEFAULTPARSER, FALSE);
		if (empty($key)) {
	...
	private function twig_ext_create_insert_custom_field_keyvalue($pageid, $key="", $value="", $debug=FALSE) {
		...
		if (is_string($value)) {
			...
			$valueout = serialize($value);
		}
		$cpfinsertstatus = NULL;
		if (!empty($key)) {
			$cpfinsertstatus = update_post_meta($pageid, $key, $value);
			$this->debug_out("set CPF $pageid used", "$key : $valueout, Status: $cpfinsertstatus"); 
		} else {
			$this->debug_out("set CPF $pageid not SET (empty key or value)", "$key : $valueout"); 
		}
		$this->debug = FALSE;
	...
	private function twig_ext_get_files_in_dir($dir="") {
		$fileList = Array();
		if (empty($dir) || (!is_dir($dir))) {
			$fileList["error"] = "invalid dir";
			$fileList["dir"] = $dir;
			return $fileList;
		}
		
		if (!is_readable($dir)) {
			$fileList["error"] = "no permission to read dir";
			$fileList["dir"] = $dir;
			return $fileList;
		}
		
		$fileListArr = scandir($dir);
		if (is_array($fileListArr)) {
			$fileListArr = array_diff($fileListArr, array('..', '.'));
			return $fileListArr;
		}
		$fileList["error"] = "scan of dir failed";
		$fileList["dir"] = $dir;
		return $fileList;
	}


	private function twig_ext_get_memory($mempeak=FALSE) {
		if ($mempeak) {
			$memory = function_exists('memory_get_peak_usage') ? round(memory_get_peak_usage(true) / 1024 / 1024, 2) : 0;
		} else {
			$memory = function_exists('memory_get_usage') ? round(memory_get_usage(true) / 1024 / 1024, 2) : 0;
		}
		return $memory;
	}
	...
	private function twig_ext_mediastore($data, $parentPageId="", $mediatitle="", $postslug= "", $content="", $excerpt="", $publishdate="", $postStatusUsed="", $authorid="", 
		...
		$fl = "";
		try {
			$fi = file_get_contents($data, false, $loadContext_stream_context);
			if (empty($fi)) {
				throw new Exception("failed to open stream ", 1);
			} 
		} catch (Exception $e) {
			$retval["jci"]["statusmsg"] = "http-loading failed";
			$retval["jci"]["errormessage"] = $data;
			return json_encode($retval);
		}
		$uploadfile = wp_upload_bits($filename, null, $fi); # return: file, urltype, error (bool)
		if ($uploadfile['error']) {
			$retval["jci"]["statusmsg"] = "uploading failed";
...
/*
			...
			$twig_extra_function_get_files_in_dir = new \Twig\TwigFunction('wp_get_files_in_dir', function ($dir='') {
				return $this->twig_ext_get_files_in_dir($dir);
			});
			$this->twig_environment->addFunction($twig_extra_function_get_files_in_dir);


			$twig_extra_function_get_memory = new \Twig\TwigFunction('wp_get_memory', function ($mempeak=FALSE) {
				return $this->twig_ext_get_memory($mempeak);
			});
			$this->twig_environment->addFunction($twig_extra_function_get_memory);
			...
			$twig_extra_filter_medialist = new \Twig\TwigFunction('wp_medialist', function ($pageid="", $pattern="") {
				return $this->twig_ext_medialist($pageid, $pattern);
			});
			$this->twig_environment->addFunction($twig_extra_filter_medialist);
			
			$twig_extra_filter_get_taxonomy = new \Twig\TwigFunction('wp_get_taxonomy', function ($pageid, $taxonomySlug, $debug=FALSE) {
				return $this->twig_ext_get_taxonomy($pageid, $taxonomySlug, $debug);
			});
			$this->twig_environment->addFunction($twig_extra_filter_get_taxonomy);
...
*/
			...
			$twig_extra_function_get_headers = new \JCITwig\TwigFunction('get_headers', function ($data, bool $associative = false, \resource $context = null) {  # usage: {{ get_headers("https://json-content-importer.com/extra/images/jci-logo-144x144.png") |json_encode }}
				return $this->twig_ext_get_headers($data, $associative, $context);
			});
			$this->twig_environment->addFunction($twig_extra_function_get_headers);
			
			$twig_extra_function_get_attachment_image_src = new \JCITwig\TwigFunction('get_attachment_image_src', function ($data, $size = 'thumbnail', bool $icon = false) {
				return $this->twig_ext_get_attachment_image_src($data, $size, $icon);
			});
			$this->twig_environment->addFunction($twig_extra_function_get_attachment_image_src);
		

			$twig_extra_function_get_files_in_dir = new \JCITwig\TwigFunction('wp_get_files_in_dir', function ($dir='') {
				return $this->twig_ext_get_files_in_dir($dir);
			});
			$this->twig_environment->addFunction($twig_extra_function_get_files_in_dir);

			$twig_extra_function_get_memory = new \JCITwig\TwigFunction('wp_get_memory', function ($mempeak=FALSE) {
				return $this->twig_ext_get_memory($mempeak);
			});
			$this->twig_environment->addFunction($twig_extra_function_get_memory);
			...
			$twig_extra_filter_get_taxonomy = new \JCITwig\TwigFunction('wp_get_taxonomy', function ($pageid, $taxonomySlug, $debug=FALSE) {
				return $this->twig_ext_get_taxonomy($pageid, $taxonomySlug, $debug);
			});
			$this->twig_environment->addFunction($twig_extra_filter_get_taxonomy);
			...
			$twig_extra_function_wp_set_post_categories = new \JCITwig\TwigFunction('wp_set_post_categories', function ($postid, $post_categories, $append = false) {
				return $this->twig_extra_function_wp_set_post_categories($postid, $post_categories, $append);
			});
    ...
    private function sortJsonArray($arrayIn, $sortField, $sortOrder, $sortFlag) {
      @usort($arrayIn, function ($a, $b) use ($sortField, $sortOrder, $sortFlag) {
        ...
        for ($i=1;$i<=count($sortField);$i++) {
          ...
          $valA = $a[$sortField[$i]] ?? null;
          $valB = $b[$sortField[$i]] ?? null;
