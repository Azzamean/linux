...
class jci_request_prepare {
	...
	public function twig_string($jsonArr, $twig_string, $twigVersion = JCI_DEFAULTPARSER) {
		if (
		...
		try {
			...
			return "<hr><font color=red>Invalid twig-code:<br>$twig_string_twigged</font><hr>";
		}
		if (preg_match("/^Twig-Error/", $twig_string_twigged)) {
			return "<hr><font color=red>Error - invalid twig-code:<br>$twig_string_twigged</font><hr>";
		} else {
