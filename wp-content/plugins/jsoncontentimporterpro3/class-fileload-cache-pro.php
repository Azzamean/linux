		...
		for ($j=0;$j<count($curloptionlistArr);$j++) {
			...
			if (""==$curloptValueStr) {
				...
				if (preg_match("/CURLOPT_HTTPHEADER/i", $curloptKeyStr)) {
					$curloptValueStr = preg_replace("/<p>/i", "", trim($curloptValueStr));
					$curloptValueStr = preg_replace("/<\/p>/i", "", $curloptValueStr);  # remove wpautop-tags from the string, which might come when asking the API for an token by an Shortcode
