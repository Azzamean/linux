...
class JsonContentImporterPro {
	...
	private $orderofshortcodeeval = Array(); 
	...
	public function jci_wp_head() { #put JSON-data into HTML-Head
		...
		if (isset($cpfvArr[0]) && ""!=$cpfvArr[0]) {
			...
			if (isset($this->twigHandler)) { # title shortcode from jci_setpagetitle needed!! otherwise no json-data and twig...
				...
				$jsonres = $this->twigHandler->executeTwig($this->feedDataArr, $cpfvArr[0], JCI_DEFAULTPARSER, FALSE);
			}
	...
	private function jci_showDebugMessage() {
		return logDebug::$debugmessage; # Scope Resolution Operator
	}
    private function buildDebugTextarea($message, $txt, $addline=FALSE) {
		$tmpdebugmode = $this->debugModeIsOn[$this->nestedlevel] ?? FALSE;
		$tmpdebuglevel = $this->debugLevel[$this->nestedlevel] ?? 1;
		logDebug::jci_buildDebugTextarea($message, $tmpdebugmode, $tmpdebuglevel, $txt, $addline);
    }
	private function jci_collectDebugMessage($debugMessage, $debugLevel=2, $suffix="", $convert2html=TRUE, $switchoffDebugPrefix = FALSE, $prefix="", $maxlength=400) {
		if (!class_exists("logDebug")) {
			require_once plugin_dir_path( __FILE__ ) . '/lib/logdebug.php';
		}
		$tmpdebugmode = $this->debugModeIsOn[$this->nestedlevel] ?? FALSE;
		logDebug::jci_addDebugMessage($debugMessage, $tmpdebugmode, $debugLevel, $debugLevel, $suffix, $convert2html, $switchoffDebugPrefix, $prefix, $maxlength);
    }
    ...
    private function createPage($no, $newPostType, $newPostTitle, $newPostSlugname, $newPostCategory, $content, $jci_uniquekey_createpost, $postStatusIn, $featuredImgURL, $featuredImgAlt, $featuredImgSrc, $postPublishTime="", $postauthorid="") {
		...
		*/
		...
		if (is_numeric($postdateoffsettmp)) {
			...
			$this->jci_collectCreateMessage( "($no) numeric postdateoffset ".$this->postdateoffset.": ".$postdate);
	...
	public function shortcodeExecute($atts , $content = ""){
      ...
      if ($content=="") {
        ...
        if ($thereIsAIdOrName) {
          ...
          if (is_null($tmpl)) {
			...
			$this->parser[$this->nestedlevel] = JCI_DEFAULTPARSER; # default value

			$tmpl->parser = $tmpl->parser ?? JCI_DEFAULTPARSER;
			#if (("twig"==$tmpl->parser) || ("twig243"==$tmpl->parser) || ("twig3"==$tmpl->parser) || ("twig332"==$tmpl->parser) || ("twig332adj"==$tmpl->parser) || ("twig351adj"==$tmpl->parser) ) { 
			if (("twig332adj"==$tmpl->parser) || ("twig351adj"==$tmpl->parser) ) { 
            ...
            # check template
			$tmpl->debugmode = $tmpl->debugmode ?? 1;
          ...
          $overwriteValuesFromTemplate = TRUE;
			...
			$this->parser[$this->nestedlevel] = JCI_DEFAULTPARSER;
      ...
      if (""!=$parser) {
        ...
        #if ($parser=="twig" || $parser=="twig243" || $parser=="jci" || $parser=="twig3" || $parser=="twig332" || $parser=="twig332adj" || $parser=="twig351adj") {
        if ($parser=="twig332adj" || $parser=="twig351adj") {
	  ...
	  $this->parser[$this->nestedlevel] = $this->parser[$this->nestedlevel] ?? JCI_DEFAULTPARSER;
      $this->jci_collectDebugMessage("selected parser: ".$this->parser[$this->nestedlevel]);

		#$this->parser[$this->nestedlevel] = JCI_DEFAULTPARSER; # default value
		# echo "Testing default parser:".$this->parser[$this->nestedlevel]."<hr>";
    ...
    if (is_null($this->twigHandler)) {
		...
		*/
		...
		if (preg_match("/^adj3.5.1/", $twigthere)) {
			$this->parser[$this->nestedlevel] = "twig351adj";
		}
		
	}
	
	/*
	if ($this->parser[$this->nestedlevel]=="twig" || $this->parser[$this->nestedlevel]=="twig243" || $this->parser[$this->nestedlevel]=="jci" || $this->parser[$this->nestedlevel]=="twig3" || $this->parser[$this->nestedlevel]=="twig332") {
		$tmpkey = "jci_pro_deprecated";	
		$tmpgetopt = get_option($tmpkey) ?? '[]';
		$depr = json_decode($tmpgetopt, TRUE) ?? NULL;
		if (is_null($depr)) {
			$depr = array();
		} else {
			$srvrequriTmp = $_SERVER['REQUEST_URI'] ?? 'jci';
			$srvrequriArr = explode("?", $srvrequriTmp);
			$srvrequri = $srvrequriArr[0] ?? 'jcinoqm';
			$req = md5($srvrequri);
			$depr[$req]["msg"] = "deprecated";
			$nocalls = $depr[$req]["no"] ?? 0;
			$depr[$req]["no"] = $nocalls + 1;
			$depr[$req]["parser"] = htmlentities($this->parser[$this->nestedlevel]);
			$depr[$req]["REQUEST_URI"] = $srvrequri;
			$depr[$req]["time"] = time();
			$depr[$req]["ftime"] = date("d.m.Y, H:i:s");
			
			if (count($depr)>10) {
				# a new page came along, remove the oldest item in the array
				$idtoremove = "";
				$currenttime = time();
				foreach ($depr as $key => $value) {
					$dateofitem = $value["time"];
					#echo "$key dot: $dateofitem // $currenttime<br>";
					if ($dateofitem<$currenttime) {
						$currenttime = $dateofitem;
						$idtoremove = $key;
					}
				}
				#echo "$idtoremove oldest: $currenttime<br>";
				unset($depr[$idtoremove]);
			}
		}
		$inputValPost = json_encode($depr);
		if (!($inputValPost == get_option($tmpkey))) {
			update_option($tmpkey, $inputValPost );
		}
	}
	*/
	### twig-init-end
	...
	#########
	  ...
	  if ($useValuesFromUseSet) {
		...
		$twigResult = $twigHandler->executeTwig($reducedJsonArr, $twigcontent, JCI_DEFAULTPARSER, TRUE);
...
#       if (""!=($this->urlparam4twig[$this->nestedlevel])) {
       if (!empty($this->urlparam4twig[$this->nestedlevel])) {
        ...
        for ($urlp=0; $urlp<count($urlparam4TwigArr); $urlp++) {
		  ...
		  if (is_array($fi_get)) {
			...
			$tmp = sanitize_text_field($fi_get);
			$tmpout = $tmp;
		  }
          if (empty($tmpout)) {
			$fi_post = $_POST[$fi] ?? '';
			if (is_array($fi_post)) {
				...
				$tmp = sanitize_text_field($fi_post);
				$tmpout = $tmp;
			}
            if (!empty($tmp)) {
     ...
     if (!empty($this->curloptions[$this->nestedlevel])) {
        ...
        if (preg_match("/#BRO#(.*)#BRC#/i",  $this->curloptions[$this->nestedlevel])) {
          ...
          $this->curloptions[$this->nestedlevel] = do_shortcode($this->curloptions[$this->nestedlevel]);
			# remove wpautop tags <p>..</p>
			# wpautop simulation: $this->curloptions[$this->nestedlevel] = "<p>". $this->curloptions[$this->nestedlevel]."</p>";
			#$this->curloptions[$this->nestedlevel] = preg_replace("/^<p>/i", "", trim($this->curloptions[$this->nestedlevel]));
			#$this->curloptions[$this->nestedlevel] = preg_replace("/<\/p>$/i", "", $this->curloptions[$this->nestedlevel]);
      ...
      # set cachetime END
		...
		$this->cacheFile = $cacheFolderObj->getCacheFileName($this->feedUrl[$this->nestedlevel], $this->postPayload[$this->nestedlevel], $this->postbody[$this->nestedlevel], $this->curloptions);
	...
	if ($this->isCacheEnable) {
      ...
      $fileLoadWithCacheObj = new FileLoadWithCachePro(
			...
			$this->showapiresponse[$this->nestedlevel], 
			TRUE  # vers 3.7.11: changed follow-301 from FALSE to TRUE
            );
	...
	# build json-array
    #if (($this->parser[$this->nestedlevel]=="twig") || ($this->parser[$this->nestedlevel]=="twig243") || ($this->parser[$this->nestedlevel]=="twig3") || ($this->parser[$this->nestedlevel]=="twig332") 
	#	|| ($this->parser[$this->nestedlevel]=="twig332adj")
	#	|| ($this->parser[$this->nestedlevel]=="twig351adj")
	#) {
    if (($this->parser[$this->nestedlevel]=="twig332adj")
		|| ($this->parser[$this->nestedlevel]=="twig351adj")
	) {
...
##################
		...
		$osl = $this->orderofshortcodeeval[$this->nestedlevel] ?? '';
		$modetmp = $this->mode[$this->nestedlevel] ?? '';
        if (
          ($osl==1)
          #($val_jci_pro_order_of_shortcodeeval==1)
          && ($modetmp!="create")
...
# cpttype set in shortcode, e. g. slug of Toolset-CPT
		...
		if (empty($cpttypeArr)) {
			$cpttypeArr = $this->createoptionsArr[$this->nestedlevel]['cpt'] ?? NULL;
		}
		...
		## set CPT to use: either via JCI-pluginoptions created CPT or as reference to Plugins creating CPT (e. g. Toolset)
			if ($useGeneratedCPT) {
				...
				if ("post"==$typeOfNewpage || "page"==$typeOfNewpage) {
					...
					$tmp_zorb = $zorbArr["type"] ?? '';
					if (""!=$typeOfNewpage && $typeOfNewpage==$tmp_zorb) {
...
########### look into the JSON to detect and count the values of the taxonomies
		...
		if ($useGeneratedCPT) {
			foreach ( $this->taxonomiesArr as $taxonomyByToolsetK => $taxonomyByToolsetV ) {
				$tmp1 = $cpttypeArr[0]["taxonomiesmatches"][0][$taxonomyByToolsetK] ?? '';
				@$tmp = trim($tmp1);
				if (!empty($tmp)) {
...
###############################	
			...
			if (isset($vals["urlparam"])) {
				...
				## create page
					...
					$linktonewpost = get_page_link($idOfNewPost) ?? "";
					$this->jci_collectCreateMessage("($k) <a href=".$linktonewpost." target=_blank>show created page $idOfNewPost: $linktonewpost</a>");
	...
	*/
	...
	private function select_CPF_From_CPT_ACF($cpttype){
		...
		foreach($result as $k => $v){
			$rtmp = $resultOut[$v["ID"]] ?? 0;
			$resultOut[$v["ID"]] = $rtmp + 1;
		}
		...
		foreach ($acfArr as $k => $v) {
			$rtmp = $acfArrOut[$v["post_excerpt"]] ?? 0;
			$acfArrOut[$v["post_excerpt"]] = $rtmp + 1;
		}
		return $acfArrOut;
	}
