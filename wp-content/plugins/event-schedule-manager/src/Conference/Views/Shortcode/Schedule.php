...
use TEC\Conference\Vendor\StellarWP\Dates\Dates;
use TEC\Conference\Vendor\StellarWP\Dates\Timezones;
use WP_Query;
		...
		if ( $dates !== [] ) {
			...
			if ( count( $dates ) > 1 ) {
				...
				foreach ( $dates as $date ) {
					...
					// Tab date object.
					$tab_datetime = Dates::build_date_object( $date, $timezone_obj );
					$tab_date     = $tab_datetime->format_i18n( 'l, F j' );

					$output .= '<button class="tec-tabs-list-button" role="tab"' .
						' aria-selected="' . esc_attr( $selected ) . '"' .
						' aria-controls="tec-panel-' . esc_attr( $tab_count ) . '"' .
						' id="tab-' . esc_attr( $tab_count ) . '"' .
						' data-id="' . esc_attr( $tab_count ) . '"' .
			...
			foreach ( $dates as $date ) {
				...
				if ( $attr['layout'] == 'table' ) {
					...
					foreach ( $sessions as $time => $entry ) {
						...
						$row_datetime = Dates::build_date_object( $time, $timezone_obj );
						$row_time     = $row_datetime->format_i18n( $time_format );

						$html .= sprintf( '<tr class="%s">', sanitize_html_class( 'tec-time-' . $row_time ) . $global_session . $global_session_slug );
						$html .= sprintf( '<td class="tec-time">%s</td>', str_replace( ' ', '&nbsp;', esc_html( $row_time ) ) );
						$html .= $columns_html;
						$html .= '</tr>';
					}
			...
			@media screen and (min-width:700px) {
				#tec_' . $array_times[0] . '.tec-layout-grid {
					...
					if ( $array_times !== [] ) {
						foreach ( $array_times as $array_time ) {
							$array_datetime       = Dates::build_date_object( $array_time, $timezone_obj );
							$array_time_formatted = $array_datetime->format_i18n( $time_format );

							$html .= '<h2 class="tec-time" style="grid-row: time-' . esc_html( $array_time ) . ';">' . esc_html( $array_time_formatted ) . '</h2>';
						}
					}
					...
					foreach ( $sessions_query->posts as $session ) {
						...
						$session_datetime = Dates::build_date_object( $date, $timezone_obj );
						$session_time     = $session_datetime->format_i18n( $time_format );
						$end_datetime     = Dates::build_date_object( $end_time, $timezone_obj );
						$end_time         = $end_datetime->format_i18n( $time_format );

						// Add time to the output string
						$html .= '<div class="tec-session-time">';
						$html .= esc_html( $session_time ) . ' - ' . esc_html( $end_time );
						if ( $minutes ) {
