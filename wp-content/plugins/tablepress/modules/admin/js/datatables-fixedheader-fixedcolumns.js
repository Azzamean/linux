...
$( '#tablepress_edit-datatables-fixedheader-fixedcolumns' ).addEventListener( 'change', ( event ) => {
	if ( ! event.target || ! ( event.target instanceof HTMLInputElement ) || 'checkbox' !== event.target.type ) {
		return;
	}
...
add_filter( 'tablepress.optionsLoad', 'tp/datatables-fixedheader-fixedcolumns/load-option-value', option_name => {
	...
	if ( tp.table.options.datatables_fixedcolumns_left_columns > 1 || tp.table.options.datatables_fixedcolumns_right_columns > 1 ) {
